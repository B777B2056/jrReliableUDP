...
![发送窗口](pic/snd1.png)  
发送窗口实现采用std::map容器，根据报文SEQ字段（序列号）构建有序发送缓存；该缓存中只保留SND.WND范围内的数据包，因此只保有SND.WND变量，无需保有SND.UNA和SND.NXT（这两个变量隐式的被map包含了）。具体做法如下：  
...
3. 新数据包添加进发送缓存，当发送缓存区被填满后即开始发送数据——新数据入缓存即发送窗口打开（上图中的“Open”）。  
### 3.2 接收窗口
